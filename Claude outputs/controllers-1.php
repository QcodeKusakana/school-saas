...
function ctrl_auth_forgot(): void
{
    ...
    // LE MESSAGE NE VARIE PAS SELON L'ISSUE DE L'ENVOI.
    //
    // Dire « e-mail envoyé » ou « pas d'adresse enregistrée » ferait
    // de ce formulaire un outil d'énumération de comptes — ce que tout
    // le reste de ce chemin s'emploie à empêcher. L'échec éventuel est
    // journalisé côté serveur, où l'attaquant ne le voit pas.
    if (isset($outcome['mail']) && $outcome['mail']['ok'] === false
        && $outcome['mail']['error'] !== null) {
        log_warning('Réinitialisation : message non remis', [
            'motif' => $outcome['mail']['error'],
        ]);
    }

    // En développement seulement, le lien reste affiché : il permet de
    // travailler sans serveur d'envoi configuré.
