...
**Version** : 0.8.0 — phases 1 à 6 terminées et auditées · 7A, 7B1, 7B2, 7D livrées et auditées · **8A Messagerie livrée**
**Dernière mise à jour** : 21/09/2026
**Tests** : 869 verts sur base fraîche **MySQL 8** (16 suites)
...
| 8 | **Messagerie & hors connexion** | **8A Messagerie ✅** (audit ⏭) · 8B PWA, Service Worker, IndexedDB, synchronisation | 🔵 en cours |
...
tout passe. Total attendu : **869 tests verts**.
...
- Suite complète vérifiée sur **MySQL 8.0** : 869 verts, 0 échec.
...
## Prochaine étape — audit 8A, puis la 8B (hors connexion)

La 8A est livrée. Voir `phase-8a-messagerie.md`.

**Avant toute nouvelle phase, la 8A doit être auditée par exécution.** Les
chemins à sonder, relevés en écrivant le module :

- **Deux passages du travail périodique EN MÊME TEMPS.** Rien n'empêche deux
  exécutions concurrentes de prendre le même message et de l'envoyer deux fois.
  C'est le motif de la 5B, et il n'a pas encore été mesuré ici.
- **Un message dont l'école est supprimée** pendant qu'il attend en file.
- **Un corps très long** : `MEDIUMTEXT` chiffré, encodage base64, découpage à
  76 caractères — la limite de ligne SMTP est de 1000 octets.
- **Une adresse d'expédition d'un autre domaine** que le serveur : le message
  part-il, et que dit l'écran ?
- **La clé de chiffrement changée** entre l'écriture et l'envoi.
- **Un serveur qui accepte la connexion puis coupe** au milieu du DATA.

Ensuite :

1. La **8B (hors connexion)** — PWA, Service Worker, IndexedDB, synchronisation.
   C'est le dernier grand pilier annoncé du projet.
2. La **7C (Mobile Money)**, toujours bloquée sur les identifiants du
   prestataire.
