...
/**
 * Durée de la RÉSERVATION d'un message, en minutes.
 *
 * Doit dépasser largement le temps d'un envoi (10 s de connexion +
 * 15 s par réponse) : c'est le délai au bout duquel un message repris
 * par un travail qui a été tué sera rejoué par le suivant.
 */
const MAIL_LEASE_MINUTES = 5;
...
function mail_attempt(array $row): array
{
    ...
    // ========================================================
    //  LA RÉSERVATION — sans elle, le message part DEUX FOIS
    // ========================================================
    // Défaut mesuré à l'audit 8A : deux passages du travail
    // périodique lancés à la même seconde lisaient tous deux le même
    // message « en attente », et l'envoyaient tous deux. Le serveur
    // de test a bien reçu DEUX exemplaires. Une tâche cron qui
    // déborde sur la suivante suffit à produire ce cas.
    //
    //   > Lire une file ne réserve rien. Tant que deux lecteurs
    //   > peuvent repartir avec la même ligne, la file n'en est pas
    //   > une.
    //
    // La réservation est l'UPDATE lui-même : `rowCount()` ne vaut 1
    // que pour celui qui a gagné. Le perdant s'arrête ici, sans rien
    // envoyer et sans compter de tentative.
    //
    // ON DÉPLACE L'ÉCHÉANCE PLUTÔT QUE DE POSER UN ÉTAT « en cours ».
    // Un état supplémentaire resterait collé si le travail était tué
    // au milieu d'un envoi, et il faudrait une règle pour le
    // débloquer. Une échéance repoussée EXPIRE d'elle-même : le
    // passage suivant reprend le message, et rien ne reste coincé.
    $claimed = db_query(
        'UPDATE email_messages
            SET next_attempt_at = DATE_ADD(NOW(), INTERVAL ' . MAIL_LEASE_MINUTES . ' MINUTE)
          WHERE id = :id
    ...
    if ($claimed !== 1) {
        return [
            'ok'    => false,
            'error' => 'Message déjà pris en charge par un autre envoi en cours.',
        ];
    }
